...
func getTableNameForDependent(stmt rsql.Statement) string {
	...
	case *rsql.CreateIndexStatement:
		if s.Table != nil { return s.Table.Name }
...
func AutoMigrate(ctx context.Context, db DB, schema io.Reader, allowTableDeletes bool) error {
	return db.Txc(ctx, func(tx Tx) error {
		...
		if err != nil { return fmt.Errorf("AutoMigrate: could not get master rows from DB: %w", err) }
		for _, row := range dbMasterRows {
			if strings.HasPrefix(row.Name, "sqlite_") { continue }
			parser := rsql.NewParser(strings.NewReader(row.Sql))
			stmt, parseErr := parser.ParseStatement()
			if parseErr != nil {
				if strings.Contains(row.Name, "sqlite_sequence") { continue }
		...
		for {
			...
			if errors.Is(parseErr, io.EOF) { break }
			if parseErr != nil { return fmt.Errorf("AutoMigrate: could not parse statement from input schema: %w", parseErr) }
		...
		for _, sStmt := range schemaStmtsInOrder {
			...
			if nameErr != nil { return fmt.Errorf("AutoMigrate: could not extract name from schema statement %s: %w", sStmt.String(), nameErr) }
			sNameLower := strings.ToLower(sNameOriginal)
			if _, exists := schemaObjectsMap[sNameLower]; exists { return fmt.Errorf("AutoMigrate: duplicate object name '%s' found in schema definition", sNameOriginal) }
		...
		for _, sStmt := range schemaStmtsInOrder {
			...
			if !dExistsInDbInitially { 
				...
				if sIsTable { rebuiltTables[sNameLower] = true }
			} else { 
				matchType, diffDescription, cmpErr := compareStatements(dStmt, sStmt)
				if cmpErr != nil { return fmt.Errorf("AutoMigrate: error comparing object '%s': %w", sNameOriginal, cmpErr) }
				...
				if forceRecreateDueToDependency { 
					...
					switch dStmt.(type) {
						case *rsql.CreateTableStatement: dropSQLForRecreate = fmt.Sprintf("DROP TABLE %s", qOriginalDNameForDrop)
						case *rsql.CreateIndexStatement: dropSQLForRecreate = fmt.Sprintf("DROP INDEX %s", qOriginalDNameForDrop)
						case *rsql.CreateViewStatement: dropSQLForRecreate = fmt.Sprintf("DROP VIEW %s", qOriginalDNameForDrop)
						case *rsql.CreateTriggerStatement: dropSQLForRecreate = fmt.Sprintf("DROP TRIGGER %s", qOriginalDNameForDrop)
						default: return fmt.Errorf("AutoMigrate: unknown type for DB object %s to drop for forced recreate", originalDNameForDrop)
					}
					...
					case statementMatchReorderNeeded:
						if schemaTableStmt, ok := sStmt.(*rsql.CreateTableStatement); ok {
							...
							qOldTableName := quoteIdent(sNameOriginal); qTempTableName := quoteIdent(tempTableName)
							if _, err := tx.Exec(fmt.Sprintf("ALTER TABLE %s RENAME TO %s", qOldTableName, qTempTableName)); err != nil { return fmt.Errorf("AutoMigrate: error renaming table %s to %s for reorder: %w", sNameOriginal, tempTableName, err) }
							if _, err := tx.Exec(sStmt.String()); err != nil { tx.Exec(fmt.Sprintf("ALTER TABLE %s RENAME TO %s", qTempTableName, qOldTableName)); return fmt.Errorf("AutoMigrate: error creating new table %s for reorder: %w. SQL: %s", sNameOriginal, err, sStmt.String()) }
							colNames := make([]string, len(schemaTableStmt.Columns)); for i, colDef := range schemaTableStmt.Columns { colNames[i] = quoteIdent(colDef.Name.Name) }
							joinedColNames := strings.Join(colNames, ", "); insertSQL := fmt.Sprintf("INSERT INTO %s (%s) SELECT %s FROM %s", qOldTableName, joinedColNames, joinedColNames, qTempTableName)
							if _, err := tx.Exec(insertSQL); err != nil { return fmt.Errorf("AutoMigrate: error copying data to reordered table %s: %w. SQL: %s", sNameOriginal, err, insertSQL) }
							if _, err := tx.Exec(fmt.Sprintf("DROP TABLE %s", qTempTableName)); err != nil { return fmt.Errorf("AutoMigrate: error dropping temporary table %s for reorder: %w", tempTableName, err) }
							rebuiltTables[sNameLower] = true 
						} else { return fmt.Errorf("AutoMigrate: internal error - statementMatchReorderNeeded for non-table object %s", sNameOriginal) }
					case statementMatchNoMatch:
						var dIsTable bool; if _, ok := dStmt.(*rsql.CreateTableStatement); ok { dIsTable = true }
						...
						if sIsTable && dIsTable { 
							...
							switch dStmt.(type) {
							case *rsql.CreateTableStatement: dropSQLNoMatch = fmt.Sprintf("DROP TABLE %s", qDNameOriginalForDrop)
							case *rsql.CreateIndexStatement: dropSQLNoMatch = fmt.Sprintf("DROP INDEX %s", qDNameOriginalForDrop)
							case *rsql.CreateViewStatement: dropSQLNoMatch = fmt.Sprintf("DROP VIEW %s", qDNameOriginalForDrop)
							case *rsql.CreateTriggerStatement: dropSQLNoMatch = fmt.Sprintf("DROP TRIGGER %s", qDNameOriginalForDrop)
							default: return fmt.Errorf("AutoMigrate: unknown type for DB object %s to drop for type/def change", dNameOriginalForDrop)
							}
							if _, err := tx.Exec(dropSQLNoMatch); err != nil { return fmt.Errorf("AutoMigrate: error dropping DB object %s %s for type/def change: %w", dbObjTypeForDrop, dNameOriginalForDrop, err) }
							
							if _, execErr := tx.Exec(sStmt.String()); execErr != nil { return fmt.Errorf("AutoMigrate: error creating schema object %s after dropping old version: %w. SQL: %s", sNameOriginal, execErr, sStmt.String()) }
							if sIsTable { rebuiltTables[sNameLower] = true } else if dIsTable { rebuiltTables[sNameLower] = true } 
						}
					}
				}
			}
		...
		for dNameLower, dStmt := range dbObjects { 
			if !processedSchemaObjects[dNameLower] { 
				...
				if nameErr != nil { return fmt.Errorf("AutoMigrate: error getting original name for DB object to drop (key: %s): %w", dNameLower, nameErr) }
				
				objTypeStr := getObjectType(dStmt)
				
				isTableToDrop := false
				if _, ok := dStmt.(*rsql.CreateTableStatement); ok { isTableToDrop = true }
				...
				switch dStmt.(type) {
				case *rsql.CreateTableStatement: dropSQL = fmt.Sprintf("DROP TABLE %s", qOriginalDName)
				case *rsql.CreateIndexStatement: dropSQL = fmt.Sprintf("DROP INDEX %s", qOriginalDName)
				case *rsql.CreateViewStatement: dropSQL = fmt.Sprintf("DROP VIEW %s", qOriginalDName)
				case *rsql.CreateTriggerStatement: dropSQL = fmt.Sprintf("DROP TRIGGER %s", qOriginalDName)
				default: return fmt.Errorf("AutoMigrate: unknown type for DB object %s to drop from deletion loop", originalDName)
				}
